...
function Slider() {
            ...
            {sliderItems.map((item)=>(
              ...
              <div id="slide" key={item.title} className={`relative flex flex-col md:flex-row justify-center items-center w-screen h-screen bg-[#${item.bg}]`}>
                ...
                {/* style={{height:"40%", width:"80%"}} */}
                  {/* <motion.img style={{height:"70%"}} initial={{y:-200,scale:0.7,opacity:0}} animate={{y:0,scale:1,opacity:1,delay:0.5,duration:1}} transition={{type:"spring", duration:1,delay:0.3}} src={item.img} alt="sofa" className=" md:h-[55%] z-10 object-fill"/> */}
                  {/* <motion.div  initial={{opacity:0,scale:0.2}} animate={{opacity:1,scale:1}} transition={{type:"spring", duration:0.8,delay:0.3}} className={`rounded-[100%] h-[50%] w-[80%] md:h-[50%] md:w-[53%] z-0 bg-[#${item.circleCol}] absolute`}></motion.div> */}
  ...
  <motion.div
    ...
    className={`rounded-[100%] h-[50%] w-[80%] md:h-[50%] md:w-[53%] z-0 bg-[#${item.circleCol}] absolute`}
  ></motion.div>
</div>
