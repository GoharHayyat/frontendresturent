  ...
  return (
    <>
      <motion.nav
        ...
        className="shadow-sm px-6 py-4 md:px-8  flex items-center justify-between font-sans relative"
      >
        <Link to="/">
          <span className="font-bold text-2xl md:text-2xl hover:text-teal-600">
            {" "}
            RestaurantHub.
          </span>
        </Link>
        ...
        <motion.div
          onClick={closeMenu}
          className={`pt-7 md:pt-0 pb-7 absolute z-50 bg-white shadow ${
            menu && !isCartOpen ? "" : "translate-y-[-200%]"
          } min-h-[40%] right-0 top-20 w-full md:translate-y-0 md:shadow-none md:z-0 md:flex md:bg-transparent md:pb-0 md:static md:min-h-fit md:w-auto transition-all duration-300 ease-in`}
        >
        ...
        <div className="flex gap-4">
          ...
          >
            <div className="relative">
              ...
              <span className="inline-flex absolute left-3 bottom-3 justify-center items-center w-6 h-6 text-xs font-bold text-white bg-red-500 rounded-full border-2">
                {cart.length}
              </span>
            </div>
          </motion.div>
